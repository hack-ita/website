...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===========================
  safeExecute(() => {
    if (!FEATURES.filters) return warn("Filters feature disabled");

    log("Initializing taxonomy filters");
    ...
    if (!selects.length || !posts.length)
      return warn("No filters or posts found");

    // ===========================
    // UTILITY FUNCTIONS
    // ===========================
    const anyFilterActive = (active) =>
      Object.values(active).some((v) => v !== "all");

    const getDatasetValues = (el, key) =>
      (el.dataset[key] || "")
        .split(",")
        .map((v) => v.trim())
        .filter(Boolean);
    ...
    const filter = () => {
      ...
      groups.forEach((group) => {
        if (!filtering) return (group.style.display = "");
        const hasVisible = [...group.querySelectorAll(".post-item")].some(
