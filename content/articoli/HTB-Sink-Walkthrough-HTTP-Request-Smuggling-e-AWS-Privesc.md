...
sequenceDiagram
    participant Att as Attaccante
    participant HA as HAProxy
    participant Gu as Gunicorn
    participant Vit as Vittima (admin)

    Att->>HA: Richiesta con Content-Length + Transfer-Encoding:[\x0b]chunked
    Note over HA: \x0b non riconosciuto → usa Content-Length
    HA->>Gu: Inoltra la richiesta (TE ancora scritto, non rimosso)
    Note over Gu: Rilegge TE, lo accetta → segue chunked
    Gu->>Gu: Trova il chunk 0 → chiude la prima richiesta
    Gu->>Gu: Legge POST /notes, Content-Length:290, pochi byte ricevuti → resta in attesa
    Vit->>HA: Richiesta reale (es. GET /notes/delete/...)
    HA->>Gu: Inoltra sulla stessa connessione riusata (keep-alive)
    Note over Gu: I byte della vittima riempiono lo spazio mancante di POST /notes
    Gu->>Att: La nota salvata contiene la richiesta della vittima (cookie incluso)
```
